extern crate panini_grammar_ast;

use panini_grammar_ast::token::{Token, Span};
    ...
    -> Stmts
{
    let mut parser = grammar! {
        ...
        sub enum_stream -> ((rs::Token, rs::Span)) {
            ...
            stage0_ident = ((&rs::Token::Ident(ident), _) if (ident.name.as_str() == "stage0"));
            stage1_ident = ((&rs::Token::Ident(ident), _) if (ident.name.as_str() == "stage1"));
        ...
        stage ::=
            pound l_bracket stage0_ident r_bracket l_brace stage0:stage0 r_brace => {
                Stage0::new(stage0)
            }
            | pound l_bracket stage1_ident r_bracket l_brace stage1:stage1 r_brace => {
                Stage1::new(stage1)
            };
